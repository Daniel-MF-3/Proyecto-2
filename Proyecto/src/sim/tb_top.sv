...
module tb_top;
    ...
    task inyectar_tecla(input [3:0] key);
        begin
            key_force = key;

            @(negedge clk);
            force dut.tecla_ok = key_force;
    ...
    task revisar_top(
        ...
        begin
            ...
            $display("estado=%0d n1=%0d n2=%0d suma=%0d visibles=%h%h%h%h",
                     dut.s_est, dut.n1_bin, dut.n2_bin, dut.suma_res,
                     dut.v4, dut.v3, dut.v2, dut.v1);

            if (dut.n1_bin !== exp_n1)
                $error("ERROR top n1 esperado=%0d obtenido=%0d", exp_n1, dut.n1_bin);

            if (dut.n2_bin !== exp_n2)
                $error("ERROR top n2 esperado=%0d obtenido=%0d", exp_n2, dut.n2_bin);

            if (dut.suma_res !== exp_suma)
                $error("ERROR top suma esperada=%0d obtenida=%0d", exp_suma, dut.suma_res);

            if (dut.s_est !== exp_estado)
                $error("ERROR top estado esperado=%0d obtenido=%0d", exp_estado, dut.s_est);
        end
    endtask
    ...
    initial begin
        ...
        // Espera a que bin_to_bcd convierta 1998.
        repeat (150) @(posedge clk);

        $display("Resultado BCD visible esperado 1998, obtenido %h%h%h%h",
                 dut.v4, dut.v3, dut.v2, dut.v1);
